...
fun ScanScreen(
    ...
    if (showTakeControlConfirm.value) {
        AlertDialog(
            onDismissRequest = { showTakeControlConfirm.value = false },
            ...
            confirmButton = {
                TextButton(onClick = {
                    showTakeControlConfirm.value = false
                    onTakeControl()
                }) { Text("Take control") }
            },
            dismissButton = {
                TextButton(onClick = { showTakeControlConfirm.value = false }) { Text("Cancel") }
            },
        )
    }
    ...
    AkraScreen(modifier = modifier) {
        ...
        if (state.connectionYieldState is ConnectionYieldState.Yielded) {
            AkraBanner(
                title = "Receiver in use",
                body = BleRepositoryImpl.YIELD_MESSAGE,
                accent = AkraColors.Amber,
                actionLabel = "Take control",
                onAction = { showTakeControlConfirm.value = true },
            )
        }
